...
class CustomCakeRequest(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="custom_cake_requests")
...
class Review(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="cake_reviews")
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["user", "cake"], name="one_review_per_user_cake")
        ]
        ordering = ["-created_at"]
...
class Notification(models.Model):
    ...
    phone = models.CharField(max_length=20)
    message = models.TextField()
    status = models.CharField(max_length=20, choices=STATUS, default="DEMO")
    ...
    def __str__(self):
        return f"{self.channel} - {self.status} - {self.phone}"
